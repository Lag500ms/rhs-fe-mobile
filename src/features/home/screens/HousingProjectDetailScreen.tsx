  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* ── Lottery & Deposit Section ── */}
        {(project.lotteryDate || project.lotteryLocation || project.depositAmount > 0) ? (
          <View style={styles.lotteryCard}>
            <View style={styles.lotteryCardHeader}>
              <View style={styles.lotteryIconWrap}>
                <Feather name="award" size={22} color={RHSColors.govGold} />
              </View>
              <Text style={styles.lotteryCardTitle}>Thông tin bốc thăm</Text>
            </View>
            <View style={styles.lotteryCardDivider} />
            {project.lotteryDate && (
              <View style={styles.lotteryRow}>
                <Feather name="calendar" size={18} color={RHSColors.amber700} />
                <Text style={styles.lotteryRowLabel}>Ngày bốc thăm</Text>
                <Text style={styles.lotteryRowValue}>{new Date(project.lotteryDate).toLocaleDateString('vi-VN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</Text>
              </View>
            )}
            {project.lotteryLocation && (
              <View style={styles.lotteryRow}>
                <Feather name="map-pin" size={18} color={RHSColors.blue700} />
                <Text style={styles.lotteryRowLabel}>Địa điểm</Text>
                <Text style={styles.lotteryRowValue}>{project.lotteryLocation}</Text>
              </View>
            )}
            {project.depositAmount > 0 && (
              <View style={styles.depositHighlight}>
                <View style={styles.depositLabelRow}>
                  <Feather name="credit-card" size={18} color={RHSColors.red600} />
                  <Text style={styles.depositLabel}>Số tiền đặt cọc</Text>
                </View>
                <Text style={styles.depositValue}>{project.depositAmount.toLocaleString('vi-VN')} VNĐ</Text>
                <Text style={styles.depositHint}>Số tiền này sẽ được hoàn trả nếu không trúng tuyển</Text>
              </View>
            )}
          </View>
        ) : null}
